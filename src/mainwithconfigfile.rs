...
// A new function to parse the formula string and return a conversion closure.
fn parse_formula(formula: &str) -> Box<dyn Fn(u32) -> io::Result<f32>> {
    let formula_str = formula.to_string();
    Box::new(move |val| {
        // A very simple formula parser that handles a few common patterns.
        // This can be expanded for more complex formulas.
        let result = if formula_str.contains("A*100/255") {
            (val as f32 * 100.0) / 255.0
        } else if formula_str.contains("(A*256+B)/10.0-40.0") {
            (val as i16) as f32 / 10.0 - 40.0
        } else if formula_str.contains("(A*256+B)/100.0") {
            (val as i16) as f32 / 100.0
        } else {
            return Err(Error::new(
                ErrorKind::Other,
                format!("unsupported formula: {}", formula_str),
            ));
        };
        Ok(result)
    })
...
fn create_params_table(conf: &Ini) -> io::Result<Vec<Parameter>> {
    let mut params = Vec::new();

    // Iterate through all sections to find PIDs
    for (section_name, section) in conf.iter() {
        if section_name.is_none() || section_name.unwrap() == "general" {
            continue;
        }

        let section_name_str = section_name.unwrap();
        let desc = section
            .get("description")
            .ok_or_else(|| Error::new(ErrorKind::Other, "Missing description"))?;
        let unit = section.get("unit");
        let command_str = section
            .get("command")
            .ok_or_else(|| Error::new(ErrorKind::Other, "Missing command"))?;
        let formula_str = section
            .get("convert_formula")
            .ok_or_else(|| Error::new(ErrorKind::Other, "Missing convert_formula"))?;

        let cmd = u32::from_str_radix(command_str, 16).map_err(|_| {
            Error::new(ErrorKind::InvalidData, "Invalid command format, must be hex")
        })?;

        // The reg_address and reg_address2 are not needed with this dynamic approach,
        // so we'll just set them to 0.
...
pub async fn get_param(
    ...
    // For standard OBD-II PIDs like 015B, we don't need to set headers.
    // For custom PIDs like 22b061, we do. So we send both.
    ...
    let val_str_slice = if raw_string.len() >= 4 {
        &raw_string[raw_string.len() - 4..]
    } else {
        &raw_string[raw_string.len() - 2..]
    };

    let val = u32::from_str_radix(val_str_slice, 16);
    if let Err(_) = val {
...
async fn main() -> Result<()> {
    ...
    // The params table is now created dynamically from the config file
    let params = create_params_table(&conf)?;
        ...
        'inner: loop {
            ...
            if poll_interval.elapsed() > Duration::from_secs(0) {
                ...
                for p in &params {
                    ...
                    if let Ok(_value) = get_param(&mut stream, p, &mut client).await {
                        // Success, continue.
                    } else {
                        // Error, handle it.
                        warn!("Parameter read failed for {}", p.name);
                        info!("Reconnecting...");
                        continue 'connect;
                    }
                }
