...
fn create_params_table() -> Vec<Parameter> {
    // This function has been updated with the correct OBD-II PID and conversion for SOC.
    vec![
        Parameter::new(
            "soc",
            "State of Charge",
            Some("%"),
            0x015B, // Corrected OBD command for State of Charge.
            0x000, // Placeholder, not used for standard PIDs.
            0x000, // Placeholder, not used for standard PIDs.
            Box::new(|val| {
                // Corrected conversion logic: A * 100 / 255
                // 'val' is the raw integer response from the OBD command.
                // For a single-byte response, this is the value of that byte.
...
pub async fn get_param(
    ...
    // We'll only send the command directly.
    ...
    // The response for 015B should be a single byte. We'll parse the last two characters.
    let val = u32::from_str_radix(&raw_string[raw_string.len() - 2..raw_string.len()], 16);
    if let Err(_) = val {
